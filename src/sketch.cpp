    ...
    print() try {
        ...
        if (_height) {
            if (std::holds_alternative<bool>(*_height)) {
                ...
                height_str = std::to_string(std::get<pixels_t>(*_height)) + "p"
                                                                            "x";
            }
        }
            ...
            if (std::holds_alternative<bool>(*_position)) {
                ...
                if (std::holds_alternative<bool>(pos.first) &&
                    ...
                    if (std::holds_alternative<bool>(pos.first)) {
                        ...
                        position_str +=
                            std::to_string(std::get<pixels_t>(pos.first)) + "p"
                                                                            "x";
                    }
...
const auto percent = x3::rule<struct percent, percent_t>{"percent"};
...
const auto pixels     = x3::rule<struct pixels, pixels_t>{"pixels"};
const auto pixels_def = x3::uint_[([](auto& ctx) {
                            x3::_val(ctx) =
                                static_cast<pixels_t>(x3::_attr(ctx));
                        })] >>
                        "px";
...
const auto width = x3::rule<struct width, width_t>{"width"};
const auto
...
const auto height = x3::rule<struct height, height_t>{"height"};
const auto
...
const auto horizontal = x3::rule<struct horizontal, horizontal_t>{"horizontal"};
const auto horizontal_def =
    centered[([](auto& ctx) { x3::_val(ctx) = x3::_attr(ctx); })] |
    percent[([](auto& ctx) { x3::_val(ctx) = x3::_attr(ctx); })] |
    pixels[([](auto& ctx) { x3::_val(ctx) = x3::_attr(ctx); })];

const auto vertical = x3::rule<struct vertical, vertical_t>{"vertical"};
const auto vertical_def =
    centered[([](auto& ctx) { x3::_val(ctx) = x3::_attr(ctx); })] |
    percent[([](auto& ctx) { x3::_val(ctx) = x3::_attr(ctx); })] |
    pixels[([](auto& ctx) { x3::_val(ctx) = x3::_attr(ctx); })];

const auto point = x3::rule<struct point, point_t>{"point"};
const auto point_def =
    x3::lit("position") > '=' >
    horizontal[([](auto& ctx) { x3::_val(ctx).first = x3::_attr(ctx); })] >
    ',' > vertical[([](auto& ctx) { x3::_val(ctx).second = x3::_attr(ctx); })];

const auto position     = x3::rule<struct position, position_t>{"position"};
const auto position_def = centered | point;

const auto fullscreen = x3::rule<struct fullscreen, fullscreen_t>{"fullscreen"};
const auto fullscreen_def =
    x3::lit("fullscreen")[([](auto& ctx) { x3::_val(ctx) = true; })];

const auto attribute =
    x3::rule<struct attribute,
             std::tuple<width_t, height_t, position_t, fullscreen_t>>{
        "attribute"};
